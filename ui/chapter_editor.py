...
class ChapterEditor:
    ...
    def on_chapter_select(self, event):
        ...
        try:
            ...
            if chapter:
                ...
                self.content_text.insert(1.0, chapter.text_content or '')
        except BusinessException as e:
            messagebox.showerror("错误", e.message)
    ...
    def add_chapter(self):
        ...
        try:
            ...
            dialog = ChapterEditDialog(self.parent, self.chapter_controller, self.project.id)
            if dialog.result:
                self.load_chapters()
        except BusinessException as e:
            messagebox.showerror("错误", e.message)
    ...
    def edit_chapter(self):
        ...
        try:
            ...
            if chapter:
                from app.ui.chapter_edit_dialog import ChapterEditDialog
                dialog = ChapterEditDialog(self.parent, self.chapter_controller, self.project.id, chapter)
                if dialog.result:
                    self.load_chapters()
        except BusinessException as e:
            messagebox.showerror("错误", e.message)
    ...
    def parse_lines(self):
        ...
        try:
            from app.ui.line_parse_dialog import LineParseDialog
            dialog = LineParseDialog(
                self.parent, self.current_chapter_id, self.project, self.app_controller
            )
        except BusinessException as e:
            messagebox.showerror("错误", e.message)
    ...
    def manage_lines(self):
        ...
        try:
            from app.ui.line_window import LineWindow
            line_window = LineWindow(self.parent, self.current_chapter_id, self.project, self.app_controller)
        except BusinessException as e:
            messagebox.showerror("错误", e.message)
